...
```
*(หมายเหตุ: หากตอนติดตั้งตั้งรหัสผ่านอื่นไว้ ให้เปลี่ยนคำว่า `root_password` ให้ตรงตามจริง)*
...
```
*(ระบบจะถามรหัสผ่าน ให้ป้อนรหัสผ่านที่ตั้งในหน้า Cloud Panel)*
...
2. **สั่งนำเข้าฐานข้อมูลผ่าน SSH:** รันคำสั่งนี้บนหน้าจอ SSH Terminal เพื่อดึงประวัติตารางและข้อมูลทั้งหมดเข้าสู่ฐานข้อมูล:
   ...
   ```
   *(ระบุรหัสผ่านฐานข้อมูลที่คุณตั้งไว้ เช่น `#logic1122` แล้วกด Enter)*
...
2. กดเปิดไฟล์จากพาธ: **`C:\Windows\System32\drivers\etc\hosts`** *(เปลี่ยนแถบขวาเป็น All Files เพื่อค้นหา)*
...
*⚠️ **ข้อควรระวัง:** ห้ามรันคำสั่งกลุ่มที่มีการล้างตารางทิ้งใน Server จริงเด็ดขาด เช่น `migrate:fresh` หรือ `migrate:refresh` เพราะตระกูลเหล่านี้จะสั่งลบฐานข้อมูลทั้งหมดทิ้งแล้วเริ่มใหม่ตั้งแต่ศูนย์ ซึ่งจะทำให้ข้อมูลหายถาวร*
