...
#### ขั้นตอนการเชื่อมต่อ SSH และอัปเดตระบบด้วย Git บน CloudPanel

หากต้องการเข้าไปอัปเดตโค้ดหรือจัดการระบบบนเซิร์ฟเวอร์ CloudPanel ให้ใช้ขั้นตอนดังนี้:

##### 1. เชื่อมต่อ SSH เข้าเซิร์ฟเวอร์ (รันบนเครื่องหลักของคุณ)

```bash
ssh adminssh@192.168.7.122
```

_ระบุรหัสผ่านของคุณตอนเชื่อมต่อ (เช่น `#logic1122`)_

##### 2. ย้ายเข้าไปที่โฟลเดอร์โปรเจกต์

```bash
cd htdocs/thairubbtechconnect.com
```

##### 3. ดึงโค้ดเวอร์ชันล่าสุดจาก GitHub

```bash
git pull
```

_หมายเหตุ: หาก Git ฟ้องเรื่องสิทธิ์ (dubious ownership) ให้รันคำสั่งด้านล่างนี้ก่อนปลดล็อกสิทธิ์:_

```bash
git config --global --add safe.directory /home/thairubbtechconnect/htdocs/thairubbtechconnect.com
```

##### 4. รันเชื่อมต่อข้อมูลพนักงานและเคลียร์แคช Laravel (เมื่อมีการอัปเดตไฟล์คอนฟิก/โค้ด)

```bash
# สุ่มสร้างคีย์ระบบ (หาก APP_KEY ใน .env ยังว่างอยู่)
php artisan key:generate

# เชื่อมต่อโฟลเดอร์สำหรับอัปโหลดไฟล์/รูปพนักงาน
php artisan storage:link

# เคลียร์และจดจำแคชความเร็วของเซิร์ฟเวอร์
...
##### 5. การนำเข้าฐานข้อมูลจากเครื่องขึ้นเซิร์ฟเวอร์ (ผ่าน SSH)

หากอัปโหลดไฟล์ `backup.sql` ขึ้นเซิร์ฟเวอร์สำเร็จแล้ว สามารถรันคำสั่งนี้เพื่อดึงข้อมูลทั้งหมดเข้าฐานข้อมูลโดยตรง:
